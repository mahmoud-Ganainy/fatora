...
void item_search::on_search_in_returnPressed()
{
 ...
 if(qry.first()){
	...
	QSqlQuery qry5(db);
	QSqlQuery qry4(db);
	qry4.prepare("SELECT name from supplier WHERE id = "+supplier_id+"");
	if( qry4.exec()){
		while (qry4.next())
		{
	...
	//add ui set here
     ...
     qry2.prepare("SELECT item_id , id , buy , sell  FROM sub_items WHERE int_barcode = '"+find_in+"'");
     if( qry2.exec() ){
         while (qry2.next())
         {
    ...
    if(qry2.first()){
		...
		qry6.prepare("SELECT supplier_id , name from items WHERE id = "+id+"");
			if( qry6.exec()){
				while (qry6.next())
				{
					supplier_id = qry6.value(0).toString();
        ...
        if(qry1.first()){
			...
			QSqlQuery qry5(db);
			QSqlQuery qry4(db);
			qry5.prepare("SELECT item_id , buy , sell  FROM sub_items WHERE id = "+kid+" ");
			if( qry5.exec()){
				while (qry5.next())
				{
