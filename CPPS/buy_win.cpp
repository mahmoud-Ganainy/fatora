#include "buy_win.h"
#include "ui_buy_win.h"

buy_Win::buy_Win(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::buy_Win)
{
    ui->setupUi(this);
    ...
    connect(ui->dateEdit,SIGNAL(editingFinished()),this,SLOT(enter_pressed_4()));
...
void buy_Win::showEvent(QShowEvent *event)
{
...
void buy_Win::show_search()
{
    inv_search->search_type = 0;
	inv_search->show();
    inv_search->clear_all();
}
void buy_Win::search_invoice()
{
...
void buy_Win::clearAll()
{
   ...
   ui->tableWidget->setHorizontalHeaderLabels(QStringList()<<"ID"<<"Item name"<<"international Barcode"<<"Barcode"<<"Buy Price"<<"Quantity"<<"Total"<<"Expire"<<"Sell Price");
   ui->tableWidget->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
   ui->tableWidget_2->setHorizontalHeaderLabels(QStringList()<<"ID"<<"Item name"<<"international Barcode"<<"Barcode"<<"Buy Price"<<"Quantity"<<"Total"<<"Expire"<<"Sell Price");
...
void buy_Win::on_item_name_currentIndexChanged(int index)
{
    ui->item_bala->clear();
...
{	
    ...
    if(ui->expire_check->isChecked()){
        ...
        expire_s = ui->dateEdit->date().toString("yyyy/MM/dd");
    }
    ...
    QTableWidgetItem *exp_barcode = new QTableWidgetItem();
    ...
    QTableWidgetItem *expire = new QTableWidgetItem();
    ...
    exp_barcode->setFlags(exp_barcode->flags() & ~Qt::ItemIsEditable);	
    ...
    expire->setFlags(expire->flags() & ~Qt::ItemIsEditable);
	...
	if(ui->rec_stat->currentIndex() == 0){
        ...
        ui->tableWidget->setItem(CurrentRow, 3, exp_barcode);
        ui->tableWidget->setItem(CurrentRow, 4, buy);
        ui->tableWidget->setItem(CurrentRow, 5, quan);
        ui->tableWidget->setItem(CurrentRow, 6, total);
        ui->tableWidget->setItem(CurrentRow, 7, expire);
        ui->tableWidget->setItem(CurrentRow, 8, sell);
        getTotal();
        getTotal_items();
        total_g = ui->total_rec->text();
        total_after = ui->total_after->text();
	...
	int recCount2=0;
       ...
       if(!purch_csh.exec())
		...
		QSqlQuery salesb(db);
        salesb.prepare("INSERT INTO purchases_items_g(rec_no,item_name,item_int,exp_barcode,quantity,buy_price,sell_price,total)VALUES( :rec_no, :item_name, :item_int, :barcode, :quan, :buy,:sell,:total)");
       ...
       if(!purch_csh.exec())
		...
		QSqlQuery salesb(db);
        salesb.prepare("INSERT INTO purchases_return_items_g(rec_no,item_name,item_int,exp_barcode,quantity,price,total)VALUES( :rec_no, :item_name, :item_int, :barcode, :quan, :buy,:total)");
        salesb.bindValue(":rec_no",ui->rec_no->text());
        salesb.bindValue(":item_name",ui->item_name->lineEdit()->text());
        salesb.bindValue(":item_int",ui->item_int->lineEdit()->text());
        salesb.bindValue(":barcode",ui->item_bcode->lineEdit()->text());
	...
	if(ui->rec_stat->currentIndex()==0){
        ...
        ui->tableWidget_2->setItem(CurrentRow, 3, exp_barcode);
        ui->tableWidget_2->setItem(CurrentRow, 4, buy);
        ui->tableWidget_2->setItem(CurrentRow, 5, quan);
        ui->tableWidget_2->setItem(CurrentRow, 6, total);
        ui->tableWidget_2->setItem(CurrentRow, 7, expire);
        ui->tableWidget_2->setItem(CurrentRow, 8, sell);
        QSqlQuery salesb(db);
        ...
        salesb.bindValue(":item_name",ui->item_name->lineEdit()->text());
        salesb.bindValue(":item_int",ui->item_int->lineEdit()->text());
        salesb.bindValue(":barcode",ui->b_code->text());
        salesb.bindValue(":quan",ui->quantity->text().toDouble());
        salesb.bindValue(":buy",ui->price_buy->text().toDouble());
		salesb.bindValue(":sell",ui->price_sell->text().toDouble());
        salesb.bindValue(":total",ui->price_buy->text().toDouble()*ui->quantity->text().toDouble());
...
void buy_Win::enter_pressed_2(){
    setData_int();
}
void buy_Win::enter_pressed_3(){
    setData_bcode();
    ui->quantity->setFocus();
    ui->quantity->selectAll();
}
void buy_Win::enter_pressed_4(){
    ui->price_sell->setFocus();
    ui->price_sell->selectAll();
}
void buy_Win::slotGetPrintCallbackData(const LimeReport::CallbackInfo &info, QVariant &data)
{
    QStringList columns;
...
void buy_Win::print_bcode(){
    ...
    report->loadFromFile(":/Rc/Xml/large_bcode.lrxml");
...
void buy_Win::print_save(){
    print_bcode();
    save_items();

}
void buy_Win::save_items(){
    int rowCount = ui->tableWidget->rowCount();
	int rowCount_g= ui->tableWidget_2->rowCount();
	...
	int recCount2=0;
       ...
       if(ui->cash->isChecked()){
       QSqlQuery purch_csh(db);
       purch_csh.prepare("INSERT INTO purchases(rec_no,rec_custom,supplier_id,total,discount,payment_method_id,payment_method,storage_id,branch_id,cash_deposit_id,currency_id,employee_id,create_date)VALUES(:purch_id,:rec_no,:supp_id,:total_after,:discount,0,:payment,:stock_id,:branch,:cash_depo,:currency,:employee,:create_date)");
       purch_csh.bindValue(":purch_id",purch_id);
       purch_csh.bindValue(":rec_no",rec_no);
       purch_csh.bindValue(":supp_id",supp_id);
       ...
       purch_csh.bindValue(":currency",currency_id);
       purch_csh.bindValue(":employee",cashier_id);
       purch_csh.bindValue(":create_date",create_date);
	   ...
	   qDebug()<<purch_csh.lastError();}
        ...
        QSqlQuery purch_crdt(db);
        purch_crdt.prepare("INSERT INTO purchases(rec_no,rec_custom,supplier_id,total,discount,payment_method_id,payment_method,storage_id,branch_id,cash_deposit_id,currency_id,employee_id,create_date)VALUES(:purch_id,:rec_no,:supp_id,:total_after,:discount,1,:payment,:stock_id,:branch,:cash_depo,:currency,:employee,:create_date)");
        purch_crdt.bindValue(":purch_id",purch_id);
        purch_crdt.bindValue(":rec_no",rec_no);
        purch_crdt.bindValue(":supp_id",supp_id);
        ...
        purch_crdt.bindValue(":currency",currency_id);
        purch_crdt.bindValue(":employee",cashier_id);
        purch_crdt.bindValue(":create_date",create_date);
	   ...
	   qDebug()<<supp_bala.lastError();}
        ...
        QSqlQuery purch_chqs(db);
        purch_chqs.prepare("INSERT INTO purchases(rec_no,rec_custom,supplier_id,total,discount,payment_method_id,payment_method,storage_id,branch_id,cash_deposit_id,currency_id,employee_id,create_date)VALUES(:purch_id,:rec_no,:supp_id,:total_after,:discount,2,:payment,:stock_id,:branch,:cash_depo,:currency,:employee,:create_date)");
        purch_chqs.bindValue(":purch_id",purch_id);
        purch_chqs.bindValue(":rec_no",rec_no);
        purch_chqs.bindValue(":supp_id",supp_id);
        ...
        purch_chqs.bindValue(":currency",currency_id);
        purch_chqs.bindValue(":employee",cashier_id);
        purch_chqs.bindValue(":create_date",create_date);
    ...
    while (x<rowCount) {
       ...
       buy_price = ui->tableWidget->item(x,4)->text();
       quan = ui->tableWidget->item(x,5)->text();
       total_price = ui->tableWidget->item(x,6)->text();
       expire = ui->tableWidget->item(x,7)->text();
       sell_price = ui->tableWidget->item(x,8)->text();
       ...
       qry.bindValue(":ex_bcode",ex_bcode);
       ...
       QSqlQuery bala(db);
       bala.prepare("INSERT INTO item_balance(sub_item_id,exp_barcode,expire,storage_id,branch_id, item_balance,purchase_balance)VALUES(:id,:ex_bcode,:expire,:stock_id,:branch,:quan,:quan)");
       bala.bindValue(":id",id);
       bala.bindValue(":ex_bcode",ex_bcode);
       bala.bindValue(":quan",quan);
       bala.bindValue(":expire",expire);
       ...
       bala2.bindValue(":ex_bcode",ex_bcode);
       ...
       if(!bala2.exec())
           QMessageBox::warning(this,tr("error Updating"),bala2.lastError().text(),QMessageBox::Ok);
                    ...
                    QSqlQuery purchb(db);
                    purchb.prepare("INSERT INTO purchases_items(rec_no,item_name,item_int,exp_barcode,buy_price,quantity,total,sell_price)VALUES(:purch_id,:item_name,:item_int,:ex_code,:prch_price,:prch_quan,:prch_total,:sell_price)");
                    purchb.bindValue(":purch_id",purch_id);
                    purchb.bindValue(":item_name",item_name);
                    purchb.bindValue(":item_int",item_int);
                    purchb.bindValue(":ex_code",ex_bcode);
	   ...
	   QSqlQuery qry5(db);
        ...
        int recCount2=0;
         ...
         if(recCount2==0){
             if(ui->cash->isChecked()){
             QSqlQuery purch_csh(db);
             purch_csh.prepare("INSERT INTO purchases_return(rec_no,rec_custom,supplier_id,total,discount,payment_method_id,payment_method,storage_id,branch_id,cash_deposit_id,currency_id,employee_id,create_date)VALUES(:purch_id,:rec_no,:supp_id,:total_after,:discount,0,:payment,:stock_id,:branch,:cash_depo,:currency,:employee,:create_date)");
             purch_csh.bindValue(":purch_id",purch_id);
             purch_csh.bindValue(":rec_no",rec_no);
             purch_csh.bindValue(":supp_id",supp_id);
             purch_csh.bindValue(":total",total);
             ...
             if(!purch_csh.exec())
              ...
              QSqlQuery purch_crdt(db);
              purch_crdt.prepare("INSERT INTO purchases_return(rec_no,rec_custom,supplier_id,total,discount,payment_method_id,payment_method,storage_id,branch_id,cash_deposit_id,currency_id,employee_id,create_date)VALUES(:purch_id,:rec_no,:supp_id,:total_after,:discount,1,:payment,:stock_id,:branch,:cash_depo,:currency,:employee,:create_date)");
              purch_crdt.bindValue(":purch_id",purch_id);
              purch_crdt.bindValue(":rec_no",rec_no);
              purch_crdt.bindValue(":supp_id",supp_id);
              purch_crdt.bindValue(":total",total);
             ...
             if(!supp_bala.exec())
              ...
              QSqlQuery purch_chqs(db);
              purch_chqs.prepare("INSERT INTO purchases_return(rec_no,rec_custom,supplier_id,total,discount,payment_method_id,payment_method,storage_id,branch_id,cash_deposit_id,currency_id,employee_id,create_date)VALUES(:purch_id,:rec_no,:supp_id,:total_after,:discount,2,:payment,:stock_id,:branch,:cash_depo,:currency,:employee,:create_date)");
              purch_chqs.bindValue(":purch_id",purch_id);
              purch_chqs.bindValue(":rec_no",rec_no);
              purch_chqs.bindValue(":supp_id",supp_id);
              purch_chqs.bindValue(":total",total);
		  ...
		  while (x<rowCount) {
			   ...
			   QSqlQuery bala2(db);
				   ...
				   if(!bala2.exec())
					   QMessageBox::warning(this,tr("error Updating"),bala2.lastError().text(),QMessageBox::Ok);
                    QSqlQuery purchb(db);
                    purchb.prepare("INSERT INTO purchases_return_items(rec_no,item_name,item_int,exp_barcode,price,quantity,total)VALUES(:purch_id,:item_name,:item_int,:ex_code,:prch_price,:prch_quan,:prch_total)");
                    purchb.bindValue(":purch_id",purch_id);
                    purchb.bindValue(":item_name",item_name);
                    purchb.bindValue(":item_int",item_int);
                    purchb.bindValue(":ex_code",ex_bcode);
...
void buy_Win::on_quantity_returnPressed()
{
    if(ui->expire_check->isChecked()){
    if(!ui->item_name->lineEdit()->text().isEmpty()){
    setTable();
    clearLines();}
        ...
        ui->price_buy->setFocus();
        ui->price_buy->selectAll();
    }

}

void buy_Win::on_price_buy_returnPressed()
{
    ui->dateEdit->setFocus();
    ui->dateEdit->selectAll();
}

void buy_Win::on_price_sell_returnPressed()
{
    if(!ui->item_name->lineEdit()->text().isEmpty()){
    setTable();
    clearLines();}
...
void buy_Win::on_expire_check_stateChanged(int arg1)
{
    if(ui->expire_check->isChecked()){
        ui->dateEdit->setEnabled(false);
        if(!ui->item_bcode->lineEdit()->text().isEmpty()){
            ui->b_code->setText(ui->item_bcode->lineEdit()->text());}
        else{
            qDebug()<<"empty";
        }
    }
        ...
        ui->item_bala->clear();
        int code_id = ui->item_bcode->lineEdit()->text().toInt();
        int month = ui->dateEdit->date().month();
        int year =  ui->dateEdit->date().year()-2000;
        QString bcode_expire =  QString::number(code_id)+"-"+QString::number(month)+"/"+QString::number(year);
        ui->b_code->setText(bcode_expire);
...
void buy_Win::on_discount_editingFinished()
{
...
void buy_Win::on_total_rec_textChanged(const QString &arg1)
{
    double total_b = ui->total_rec->text().toDouble();
    double disc = ui->discount->text().toDouble();
    double total = total_b - disc;
...
void buy_Win::on_discount_returnPressed()
{
    double total_b = ui->total_rec->text().toDouble();
    double disc = ui->discount->text().toDouble();
    double total = total_b - disc;
...
void buy_Win::on_dicount_perc_editingFinished()
{
    double total_b = ui->total_rec->text().toDouble();
...
void buy_Win::on_dicount_perc_returnPressed()
{
    double total_b = ui->total_rec->text().toDouble();
    double perc = ui->dicount_perc->text().toDouble()/100;
    double disc = perc * total_b;
...
void buy_Win::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
    if(item->column()==5&&ui->tableWidget->rowCount()!=0){
        getTotal_items();
        double price = ui->tableWidget->item(item->row(),4)->text().toDouble();
        double quan = ui->tableWidget->item(item->row(),5)->text().toDouble();
        double total = price * quan;
        QTableWidgetItem *total1;
        total1 = new QTableWidgetItem;
        total1->setData(0,total);
        ui->tableWidget->setItem(item->row(),6,total1);
        getTotal();
    }
}

void buy_Win::on_save_clicked()
{
    save_items();
}

void buy_Win::on_saved_clicked()
{
    set_unedit();
	saved = true;
    if(ui->rec_stat->currentIndex()==0){
...
void buy_Win::on_rec_no_textChanged(const QString &arg1)
{
    ...
    ui->total_quan->setText(QString::number(item_count));
        ...
        model8->setQuery("SELECT name,id FROM currency ",db);
        model3->setQuery("SELECT name,id FROM cash_deposit",db);
		qry1.prepare("SELECT item_name,item_int,exp_barcode,quantity,buy_price,total,sell_price from purchases_items_g where rec_no = "+id+" ");
        if (qry1.exec())
        {
            while (qry1.next())
            {
                ...
                expire->setFlags(expire->flags() & ~Qt::ItemIsEditable);
                quan->setFlags(total->flags() & Qt::ItemIsEditable);
                QString exp_code = qry1.value(2).toString();
                QStringList lisst = exp_code.split("-");
                if(lisst.size() == 1){
                    barcode->setText(lisst.at(0));
                    expire->setText("");
                }else if (lisst.size() == 2) {
                    barcode->setText(lisst.at(0));
                    expire->setText(lisst.at(1));
                }
				item_name->setText(qry1.value(0).toString());
				int_code->setText(qry1.value(1).toString());
				exp_barcode->setText(qry1.value(2).toString());
				quan->setText(qry1.value(3).toString());
                buy->setText(qry1.value(4).toString());
				total->setText(qry1.value(5).toString());				
				sell->setText(qry1.value(6).toString());
                ...
                ui->tableWidget_2->setItem(ui->tableWidget_2->rowCount()-1, 3, exp_barcode);
                ui->tableWidget_2->setItem(ui->tableWidget_2->rowCount()-1, 4, buy);
                ui->tableWidget_2->setItem(ui->tableWidget_2->rowCount()-1, 5, quan);
                ui->tableWidget_2->setItem(ui->tableWidget_2->rowCount()-1, 6, total);
                ui->tableWidget_2->setItem(ui->tableWidget_2->rowCount()-1, 7, expire);
                ui->tableWidget_2->setItem(ui->tableWidget_2->rowCount()-1, 8, sell);
			}
		}
        ui->tableWidget_2->setHorizontalHeaderLabels(QStringList()<<"ID"<<"Item name"<<"international Barcode"<<"Barcode"<<"Buy Price"<<"Quantity"<<"Total"<<"Expire"<<"Sell Price");
        ui->tableWidget_2->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
        ...
        model8->setQuery("SELECT name,id FROM currency ",db);
        model3->setQuery("SELECT name,id FROM cash_deposit",db);
        qry1.prepare("SELECT item_name,item_int,exp_barcode,quantity,total,price from purchases_return_items_g where rec_no = "+id+" ");
        if (qry1.exec())
        {
            while (qry1.next())
            {
                ...
                quan->setFlags(total->flags() & Qt::ItemIsEditable);
                QSqlQuery qry3(db);
                qry3.prepare("SELECT sub_item_id,expire FROM item_balance WHERE exp_barcode = "+qry1.value(2).toString()+"");
                if (qry3.exec())
                {
                    while (qry3.next())
                    {
                     barcode->setText(qry3.value(0).toString());
                     expire->setText(qry3.value(1).toString());
                    }
                }
                ...
                exp_barcode->setText(qry1.value(2).toString());
                quan->setText(qry1.value(3).toString());
                buy->setText(qry1.value(5).toString());
        ...
        ui->tableWidget_2->setHorizontalHeaderLabels(QStringList()<<"ID"<<"Item name"<<"international Barcode"<<"Barcode"<<"Buy Price"<<"Quantity"<<"Total"<<"Expire");
        ui->tableWidget_2->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
...
void buy_Win::on_pushButton_clicked()
{
    if(!searching){
    if(ui->rec_stat->currentIndex()==0){
    QSqlQuery qry(db);
...
void buy_Win::on_pushButton_2_clicked()
{
   if(!searching){
    if(ui->rec_stat->currentIndex()==0){
    QSqlQuery qry(db);
...
void buy_Win::on_pushButton_4_clicked()
{
    clearAll();

}
void buy_Win::on_dateEdit_dateChanged(const QDate &date)
{
    ui->item_bala->clear();
    QString code_id = ui->item_bcode->lineEdit()->text();
    int month = ui->dateEdit->date().month();
    int year =  ui->dateEdit->date().year()-2000;
    QString bcode_test = code_id +"-"+ QString::number(month)+"/"+ QString::number(year);
    ui->b_code->setText(bcode_test);
    QString ex_code = ui->b_code->text();
    QSqlQuery qry2(db);
    qry2.prepare("SELECT item_balance FROM item_balance WHERE exp_barcode = :id ");
    qry2.bindValue(":id",ex_code);
...
void buy_Win::on_pushButton_5_clicked()
{
    print_save();
}
...
void buy_Win::on_rec_stat_currentIndexChanged(int index)
{
    int saved_count;
    int unsaved_count;
    if(index==0){
        ...
        ui->tableWidget->setHorizontalHeaderLabels(QStringList()<<"ID"<<"Item name"<<"international Barcode"<<"Barcode"<<"Buy Price"<<"Quantity"<<"Total"<<"Expire");
     if(ui->tableWidget->isVisible()){
     qDebug()<<"Salesrt";
     db.close();
     db.open();
...
void buy_Win::on_item_bcode_activated(const QString &arg1)
{
    setData_bcode();
}

void buy_Win::on_item_int_activated(const QString &arg1)
{
    setData_int();
}

void buy_Win::on_un_save_clicked()
{
    QStringList code;
    QStringList quan;
    QString id = ui->rec_no->text();
    ...
    tr("Are You Sure You WANT TO unsave!!!"), QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes)
    {
        ...
        ui->rec_no->setText(id);
            ...
            QSqlQuery qry6(db);
            QSqlQuery qry7(db);
            qry6.prepare("SELECT storage_id,branch_id,payment_method_id,total,supplier_id from purchases_return where rec_no = "+ui->rec_no->text()+" ");
            if (qry6.exec())
            {
                while (qry6.next())
                {
...
void buy_Win::on_tableWidget_clicked(const QModelIndex &index)
{
    cur_row = index.row();
    qDebug()<<cur_row;
}

void buy_Win::on_tableWidget_2_clicked(const QModelIndex &index)
{
    cur_row = index.row();
    qDebug()<<cur_row;
}

void buy_Win::delete_row()
    {
    if(ui->tableWidget->isVisible()){
...
void buy_Win::on_pushButton_3_clicked()
{
    if (QMessageBox::question(this, tr("delete?!!!"),
        ...
        if(ui->tableView->isVisible()){
           ...
           if(ui->tableWidget->rowCount()>0){
               if(ui->rec_stat->currentIndex() == 0){
                QSqlQuery qry2(db);
                qry2.prepare("Delete from purchases_g where rec_no = "+ui->rec_no->text()+" ");
                if (qry2.exec())
                { }
                QSqlQuery qry5(db);
                qry5.prepare("Delete from purchases_items_g where rec_no = "+ui->rec_no->text()+" ");
                if (qry5.exec())
                { }
                clearAll();
               }else if (ui->rec_stat->currentIndex() == 1) {
                   QSqlQuery qry2(db);
                   qry2.prepare("Delete from purchases_return_g where rec_no = "+ui->rec_no->text()+" ");
                   if (qry2.exec())
                   { }
                   QSqlQuery qry5(db);
                   qry5.prepare("Delete from purchases_return_items_g where rec_no = "+ui->rec_no->text()+" ");
                   if (qry5.exec())
                   { }
                   clearAll();
               }
...
void buy_Win::on_supp_name_currentIndexChanged(int index)
{
...
void buy_Win::on_pushButton_6_clicked()
{
...
void buy_Win::slotGetCallbackData(const LimeReport::CallbackInfo &info, QVariant &data)
{
    QSqlQuery qry2(db2);
    ...
    QStringList columns;
    columns << "rec_no" << "supplier" << "total_before" << "discount"<<"total"<<"date"<<"shop_name"<<"phone"<<"tax"<<"trade";
    switch (info.dataType) {
    case LimeReport::CallbackInfo::RowCount:
        data = 1;
        break;
...
void buy_Win::on_pushButton_7_clicked()
{
    /*report = new LimeReport::ReportEngine(this);
...
void buy_Win::on_pushButton_9_clicked()
{
    if(ui->tableWidget_2->isHidden()){
        set_edit();
    if(ui->rec_stat->currentIndex()==0){
    ...
    ui->rec_no->setText(QString::number(id));
        ...
        ui->tableWidget_2->show();
        ui->tableWidget_2->setRowCount(0);
        ui->tableWidget_2->setHorizontalHeaderLabels(QStringList()<<"Item name"<<"Barcode"<<"Buy Price"<<"Quantity"<<"Total"<<"Expire");
    QSqlQuery qry(db);
    int id;
    QDateTime date;
    QString payment;
...
void buy_Win::on_pushButton_8_clicked()
{
   /* QString printer_name;
    QSqlQuery qry(db2);
    qry.prepare("SELECT print_name from print_info WHERE print_type = 1");
...
void buy_Win::on_pushButton_10_clicked()
{
