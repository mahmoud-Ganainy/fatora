...
#include "ui_sell_win_cut.h"

sell_win_cut::sell_win_cut(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::sell_win_cut)
{
    ui->setupUi(this);
...
void sell_win_cut::showEvent(QShowEvent *event){
...
void sell_win_cut::test(){

    qDebug()<<"hhdddddddh";
}

void sell_win_cut::update_time()
{
        if(saved == false){
...
void sell_win_cut::update_itemsList()
{
    clearLines();
    db.close();
    db.open();
...
void sell_win_cut::search_invoice()
{
...
sell_win_cut::~sell_win_cut()
{
    delete ui;
}

void sell_win_cut::setData()
{
...
void sell_win_cut::on_item_name_currentIndexChanged(int index)
{
	
     int row = ui->item_name->currentIndex();
     code_id = model4->index(row,4).data().toInt();
     QString int_code = model4->index(row,3).data().toString();
	 QList<QModelIndex> modelIndexList;
     modelIndexList.clear(); 
...
void sell_win_cut::on_ex_bcode_activated(const QString &arg1)
{
    enterBcode();
}

void sell_win_cut::on_item_int_activated(const QString &arg1)
{
    enterInt();
}

void sell_win_cut::clearAll()
{
    ui->item_int->hide();
   saved = false;
   searching = false;
   ...
   if(saved_count>unsaved_count  ){
       ...
       ui->custom_rec->show();
       ui->custom_check->show();
       ui->custom_rec->setEnabled(false);
       QSqlQuery qry(db);
    ...
    if(ui->cash->isChecked()){
	 ...
	 if(ui->rec_stat->currentIndex() == 0){
        ...
        if(!sales_csh.exec())
            ...
            custom_bala.bindValue(":total",total_g);
            if(!custom_bala.exec())
...
void sell_win_cut::getTotal_items()
{
	if(ui->tableWidget_2->isVisible()){
    int column = 3;
    double total = 0 ;
...
void sell_win_cut::print_recp(){
    ...
    if(!ui->tableWidget_2->isHidden()){
        ...
        report->dataManager()->addModel("sale2",ui->tableView->model(),false);
        qDebug()<<"asd";
    }
    else if (ui->tableView_2->isVisible()) {
               report->dataManager()->addModel("sale2",ui->tableView_2->model(),false);
           }


    if(ui->tableWidget->isVisible()||ui->tableWidget_2->isVisible()){
        report->loadFromFile(":/Rc/Xml/sale_small.lrxml");
    }else{
        report->loadFromFile(":/Rc/Xml/sale_small_2.lrxml");
    }
   // report->designReport();

   report->printReport(rec_print);
...
void sell_win_cut::print_save(){
    print_recp();
    save();
}


void sell_win_cut::on_rec_stat_currentIndexChanged(int index)
{
...
void sell_win_cut::on_sell_price_textChanged(const QString &arg1)
{
...
void sell_win_cut::on_quan_textChanged(const QString &arg1)
{
    double quan = ui->quan->text().toDouble();
    if(quan<=item_bala){
        double price = ui->sell_price->text().toDouble();
...
void sell_win_cut::on_discount_per_textChanged(const QString &arg1)
{
...
void sell_win_cut::on_quan_returnPressed()
{
    if(!ui->item_name->lineEdit()->text().isEmpty()){
     setTable();
    }
...
void sell_win_cut::on_sell_price_returnPressed()
{
    ui->discount_per->setFocus();
    ui->discount_per->selectAll();
}

void sell_win_cut::on_discount_per_returnPressed()
{
    if(!ui->item_name->lineEdit()->text().isEmpty()){
     setTable();
    }
...
void sell_win_cut::on_save_clicked()
{
...
void sell_win_cut::on_dicount_returnPressed()
{
...
void sell_win_cut::on_discount_perc_editingFinished()
{
...
void sell_win_cut::on_discount_perc_returnPressed()
{
    double total_b = ui->total_before->text().toDouble();
    double perc = ui->discount_perc->text().toDouble()/100;
    double disc = perc * total_b;
...
void sell_win_cut::on_total_before_textChanged(const QString &arg1)
{
    double total_before = ui->total_before->text().toDouble();
    double discount = ui->dicount->text().toDouble();
    double total_after = total_before - discount;
...
void sell_win_cut::on_pushButton_6_clicked()
{
    print_recp();
    save();
}
...
void sell_win_cut::on_saved_rec_clicked()
{
    set_unedit();
...
void sell_win_cut::on_rec_no_textChanged(const QString &arg1)
{
...
void sell_win_cut::on_back_rec_clicked()
{
    if(!searching){
...
void sell_win_cut::on_next_rec_clicked()
{
    if(!searching){
    if(ui->rec_stat->currentIndex()==0){
    QSqlQuery qry(db);
...
void sell_win_cut::on_new_rec_clicked()
{
    clearAll();
    set_edit();
...
void sell_win_cut::on_dicount_textChanged(const QString &arg1)
{
    double total_before = ui->total_before->text().toDouble();
    double discount = ui->dicount->text().toDouble();
    double total_after = total_before - discount;
...
void sell_win_cut::clear_test(){
    clearAll();
}

void sell_win_cut::on_pushButton_clicked()
{
    QStringList code;
    QStringList quan;
    QString id = ui->rec_no->text();
    ...
    tr("Are You Sure You WANT TO unsave!!!"), QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes)
    {
        ...
        QSqlQuery qry3(db);
        qry3.prepare("INSERT INTO sales_items_g SELECT NULL,`rec_no`,`item_name`,`item_int`,`exp_barcode`,`price`,`quantity`,`discount_per`,`total`,`create` FROM sales_items WHERE rec_no = "+ui->rec_no->text()+" ");
        if (qry3.exec())
        { }              
        QSqlQuery qry4(db);
        ...
        ui->rec_no->setText(id);
            ...
            { }
            QSqlQuery qry3(db);
            qry3.prepare("INSERT INTO sales_return_items_g SELECT NULL,`rec_no`,`item_name`,`item_int`,`exp_barcode`,`price`,`quantity`,`discount_per`,`total`,`create` FROM sales_return_items WHERE rec_no = "+ui->rec_no->text()+" ");
            if (qry3.exec())
            { }
            QSqlQuery qry4(db);
...
void sell_win_cut::on_tableWidget_2_clicked(const QModelIndex &index)
{
    cur_row = index.row();
}
void sell_win_cut::on_tableWidget_clicked(const QModelIndex &index)
{
    cur_row = index.row();
    qDebug()<<cur_row;

}
void sell_win_cut::delete_row()
{
    if(ui->tableWidget_2->isVisible()){
...
void sell_win_cut::on_unsaved_clicked()
{
    if(ui->tableWidget->isHidden()){
        set_edit();
...
void sell_win_cut::on_pushButton_2_clicked()
{
...
void sell_win_cut::on_custom_name_currentIndexChanged(int index)
{
...
void sell_win_cut::on_pushButton_3_clicked()
{
...
void sell_win_cut::on_tableWidget_2_itemChanged(QTableWidgetItem *item)
{
...
void sell_win_cut::slotGetCallbackData(const LimeReport::CallbackInfo &info, QVariant &data)
{
    QSqlQuery qry2(db2);
    ...
    QStringList columns;
    columns << "rec_no" << "customer" << "total_before" << "discount"<<"total"<<"date"<<"shop_name"<<"phone"<<"tax"<<"trade"<<"time"<<"cashier";
    ...
    case LimeReport::CallbackInfo::ColumnData:
        if(info.columnName == "rec_no"){
        ...
        }else if (info.columnName == "cashier") {
            data = ui->cashier_name->currentText();
           ...
           data = ui->total_after->text();
            ...
            data = tax;
        }else if (info.columnName == "time") {
            data = QTime::currentTime();
        }
        break;
    default: break;
    }
...
void sell_win_cut::on_search_clicked()
{
    search->show();
}

void sell_win_cut::on_custom_check_stateChanged(int arg1)
{
    if(ui->custom_check->isChecked()){
        ui->custom_rec->setEnabled(true);

    }
    else if(!ui->custom_check->isChecked()) {
        ui->custom_rec->setEnabled(false);
    }
}

void sell_win_cut::on_custom_rec_returnPressed()
{
    QString rec_no = ui->custom_rec->text();
    double total,discount,discount_perc = 0;
    int counter = 0;
    QSqlQuery qry(db);
    qry.prepare("select discount,total_before from sales where rec_no = :rec_no");
    ...
    if(counter!=0){
    QSqlQuery qry1(db);
    qry1.prepare("select discount,total_before from sales where rec_no = :rec_no");
    qry1.bindValue(":rec_no",rec_no);
    if(qry1.exec()){
        while (qry1.next()) {
            discount = qry1.value(0).toDouble();
            total = qry1.value(1).toDouble();
        }
    }
    qDebug()<<discount<<total;
    discount_perc = (discount/total)*100;

    ui->discount_perc->setText(QString::number(discount_perc));
    on_discount_perc_returnPressed();
    }
