...
{	
	...
	int recCount2=0;
       ...
       if(!purch_csh.exec())
		...
		QSqlQuery salesb(db);
        salesb.prepare("INSERT INTO purchases_return_items_g(rec_no,item_name,item_int,exp_barcode,quantity,buy_price,sell_price,total)VALUES( :rec_no, :item_name, :item_int, :barcode, :quan, :buy,:sell,:total)");
        salesb.bindValue(":rec_no",ui->rec_no->text());
        salesb.bindValue(":item_name",ui->item_name->lineEdit()->text());
        salesb.bindValue(":item_int",ui->item_int->lineEdit()->text());
        salesb.bindValue(":barcode",ui->item_bcode->lineEdit()->text());
...
void buy_Win::on_checkBox_stateChanged(int arg1)
{
    if(ui->checkBox->isChecked()){        
...
void buy_Win::on_rec_no_textChanged(const QString &arg1)
{
    ...
    ui->total_quan->setText(QString::number(item_count));
        ...
        model8->setQuery("SELECT name,id FROM currency ",db);
        model3->setQuery("SELECT name,id FROM cash_deposit",db);
		qry1.prepare("SELECT item_name,item_int,exp_barcode,quantity,buy_price,total,sell_price from purchases_return_items_g where rec_no = "+id+" ");
        if (qry1.exec())
        {
            while (qry1.next())
            {
...
void buy_Win::on_un_save_clicked()
{
    ...
    tr("Are You Sure You WANT TO unsave!!!"), QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes)
    {
        ...
        QSqlQuery qry5(db);
        qry5.prepare("INSERT INTO purchases_items_g SELECT * FROM purchases_items WHERE rec_no = "+ui->rec_no->text()+" ");
        if (qry5.exec())
        {
            QSqlQuery qry7(db);
        ...
        ui->rec_no->setText(id);
            ...
            QSqlQuery qry3(db);
            qry3.prepare("INSERT INTO purchases_return_items_g SELECT * FROM purchases_return_items WHERE rec_no = "+ui->rec_no->text()+" ");
            if (qry3.exec())
            {
...
void buy_Win::on_pushButton_9_clicked()
{
    ...
    ui->rec_no->setText(QString::number(id));
        ...
        ui->tableWidget_2->show();
        ui->tableWidget_2->setRowCount(0);
        ui->tableWidget_2->setHorizontalHeaderLabels(QStringList()<<"Item name"<<"Barcode"<<"Buy Price"<<"Quantity"<<"Total"<<"Expire"<<"Sell Price");
